...
| S4.3 | Add guest endianness tracking | NOT STARTED | | | | | S4.1 | `usr.sbin/emu/emu.h`, `usr.sbin/emu/emu_engine.c` | Add `guest_endian` field to `emu_instance` struct to track target architecture endianness (EMU_ENDIAN_LITTLE/EMU_ENDIAN_BIG). |
