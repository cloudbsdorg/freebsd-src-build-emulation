...
| S13.6 | Add `--cpus` and `--sockets` flags to `emu` CLI | IN PROGRESS | freedev003 | 2026-04-27 13:45 UTC | | S13.3 | `usr.sbin/emu/emu_init.c`, `usr.sbin/emu/emu_start.c` | Added -c/--cpus N (default 1) to emu start, -s/--sockets N (default 1) to emu init and start. CLI flags override config file values. Validation against sysctl limits pending integration with kernel module. |
