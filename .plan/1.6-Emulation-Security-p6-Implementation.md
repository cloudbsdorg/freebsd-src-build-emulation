...
| # | Task | Status | Assigned To | Owner | Start | End | Dependencies | Files | Notes |
|---|------|--------|------------|-------|-------|-----|--------------|-------|-------|
| S13.1 | Detect host CPU topology (cores, sockets, threads) | NOT STARTED | | | | | | `sys/emulation/emu_smp.c`, `sys/emulation/emu.h` | Implement emu_cpu_topology_init() to detect hw.ncpu, hw.sockets, hw.cores per socket. Store in emu_host_topology struct. Used for default vCPU count. |
| S13.2 | Add `max_vcpus` and `max_sockets` sysctls | NOT STARTED | | | | | S13.1 | `sys/emulation/emu_sysctl.c`, `sys/emulation/emu_smp.c` | Implement kern.emulation.max_vcpus (default: hw.ncpu), kern.emulation.max_sockets (default: hw.sockets or 1), kern.emulation.max_vcpus_override (root-only, allows exceeding host cores). Validate against host topology. |
| S13.3 | Extend instance config with vCPU count and socket count | NOT STARTED | | | | | S13.1 | `sys/emulation/emu_instance.c`, `sys/emulation/emu.h` | Add num_vcpus, num_sockets to emu_instance_config. Default num_vcpus=1, num_sockets=1. Validate against max_vcpus/max_sockets sysctls. Store per-instance topology. |
| S13.4 | Implement per-vCPU state structures | NOT STARTED | | | | | S13.3 | `sys/emulation/emu_cpu.c`, `sys/emulation/emu.h` | Implement emu_vcpu_state struct (registers, state, APIC ID). Create array of vCPUs per instance (inst->vcpus[num_vcpus]). Each vCPU has independent state, APIC ID, socket/core/thread IDs. |
| S13.5 | Implement vCPU lifecycle (create/destroy/start/stop) | NOT STARTED | | | | | S13.4 | `sys/emulation/emu_cpu.c`, `sys/emulation/emu_instance.c` | emu_vcpu_create() allocates vCPU state, emu_vcpu_destroy() frees state, emu_vcpu_start() begins execution, emu_vcpu_stop() halts vCPU. All vCPUs start/stop with instance. |
| S13.6 | Add `--cpus` and `--sockets` flags to `emu` CLI | NOT STARTED | | | | | S13.3 | `usr.sbin/emu/emu.c`, `usr.sbin/emu/emu_start.c` | Add -c/--cpus N (default 1), -s/--sockets N (default 1) flags. Validate against sysctl limits. Store in emu_instance_config. Update usage documentation. |
| S13.7 | Implement APIC ID assignment for vCPUs | NOT STARTED | | | | | S13.4 | `sys/emulation/emu_cpu.c` | Calculate APIC ID from socket/core/thread indices. emu_calc_apic_id(socket, core, thread). Store in emu_vcpu_state. Expose via sysctl (kern.emulation.instance.<name>.vcpu.<id>.apic_id). |
