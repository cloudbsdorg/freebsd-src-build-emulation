...
|---|------|--------|------------|-------|-------|-----|--------------|-------|-------|
| S0.1 | Implement `emu_core.ko` modevent handler with instance refcount | 🔄 IN PROGRESS | freedev002 | | 2026-04-26 07:26 UTC | | 2.2 | `sys/emulation/emu_main.c` | `emu_core_modevent()`: MOD_LOAD validates no conflicts, initializes registry/sysctl/devfs. MOD_UNLOAD refuses if `emu_instance_count() > 0`. |
