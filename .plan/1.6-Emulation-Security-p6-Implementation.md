...
| # | Task | Status | Assigned To | Owner | Start | End | Dependencies | Files | Notes |
|---|------|--------|------------|-------|-------|-----|--------------|-------|-------|
| S13.1 | Implement `emu_disable_ptrace()` function | NOT STARTED | | | | | S1.4 | `usr.sbin/emu/emu_engine.c`, `emu_bhyve.c` | `procctl(PROC_TRACE_CTL_DISABLE)` + `PROC_TRACE_CTL_DISABLE_EXEC` |
| S13.2 | Write ptrace prevention tests | NOT STARTED | | | | | S13.1 | `tests/usr.sbin/emu/ptrace_test.sh` | Test that `ptrace()` to emulator process is blocked. Test that `PROC_TRACE` is restricted. |
