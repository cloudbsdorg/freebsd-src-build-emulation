...
| # | Task | Status | Assigned To | Owner | Start | End | Dependencies | Files | Notes |
|---|------|--------|------------|-------|-------|-----|--------------|-------|-------|
| S4.1 | Implement endianness-aware memory accessors | NOT STARTED | | | | | 5.2 | `usr.sbin/emu/emu_mem.h`, `usr.sbin/emu/emu_mem.c` | Add `emu_mem_read_le16/32/64`, `emu_mem_read_be16/32/64`, `emu_mem_write_le16/32/64`, `emu_mem_write_be16/32/64` using `htole*`/`letoh*`/`htobe*`/`betoh*` macros. All guest memory accesses must use these helpers. |
