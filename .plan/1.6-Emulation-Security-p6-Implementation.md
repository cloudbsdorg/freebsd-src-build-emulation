...
| S0.3 | Implement `MODULE_DEPEND` declarations for all modules | 🔄 IN PROGRESS | freedev002 | | 2026-04-26 07:46 UTC | | S0.1, S0.2 | `sys/modules/emu*/Makefile` | `emu.ko` depends on all sub-modules. Each arch module depends on `emu_core.ko`. Version checks via `MODULE_VERSION`. |
