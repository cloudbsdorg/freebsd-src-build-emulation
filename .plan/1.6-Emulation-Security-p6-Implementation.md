...
| S0.8 | Implement module unloading safety tests | 🔄 IN PROGRESS | devilsworks | | 2026-04-26 14:00 UTC | | S0.1–S0.7 | `tests/sys/emulation/module_test.c` | Test MOD_UNLOAD with active instances (expect EBUSY). Test MOD_UNLOAD with no instances (expect success). Test MOD_LOAD with conflicting modules. Test `kldload emu` loads all sub-modules. Test `kldload emu_amd64` loads emu_core automatically. |
