...
> **Workflow Reference:** Before starting any task in this document, read [`0.1-Emulation-Workflow.md`](./0.1-Emulation-Workflow.md) for instructions on task claiming, completion, and merge conflict handling.
>
> **Agent Reminder:** All task tables in this document use the standard format: `ID | Task | Status | Assigned To | Owner | Start | End | Dependencies | Files | Notes`. Claim a task by setting Status to `🔄 IN PROGRESS`, filling Assigned To and Start, then committing. Mark complete by setting Status to `✅ DONE` and filling End.
...
### 8.4 FreeBSD Ports Collection Integration

Many firmware blobs required by the emulator are **already available** in the FreeBSD ports collection. Instead of downloading them from upstream URLs, the `emu blob` tool should first check if the blob is available via ports and link to it.

#### 8.4.1 Available Ports

| Blob ID | Port Name | Port Path | Status in Ports |
|---------|-----------|-----------|-----------------|
| `seabios` | `emulators/seabios` | `/usr/ports/emulators/seabios` | ✅ Available — installs `seabios.bin` to `PREFIX/share/seabios/` |
| `ovmf-x64` | `emulators/edk2-ovmf` | `/usr/ports/emulators/edk2-ovmf` | ✅ Available — installs `OVMF_CODE.fd` and `OVMF_VARS.fd` to `PREFIX/share/edk2/` |
| `ovmf-ia32` | `emulators/edk2-ovmf-ia32` | `/usr/ports/emulators/edk2-ovmf-ia32` | ✅ Available — IA32 variant of OVMF |
| `ovmf-aarch64` | `emulators/edk2-ovmf-aarch64` | `/usr/ports/emulators/edk2-ovmf-aarch64` | ✅ Available — AArch64 variant of OVMF |
| `uboot-arm64` | `sysutils/u-boot-qemu-arm64` | `/usr/ports/sysutils/u-boot-qemu-arm64` | ✅ Available — U-Boot for QEMU arm64 |
| `uboot-arm` | `sysutils/u-boot-qemu-arm` | `/usr/ports/sysutils/u-boot-qemu-arm` | ✅ Available — U-Boot for QEMU arm |
| `uboot-riscv` | `sysutils/u-boot-qemu-riscv64` | `/usr/ports/sysutils/u-boot-qemu-riscv64` | ✅ Available — U-Boot for QEMU riscv64 |
| `opensbi` | `sysutils/opensbi` | `/usr/ports/sysutils/opensbi` | ✅ Available — OpenSBI firmware |
| `dtc` | `sysutils/dtc` | `/usr/ports/sysutils/dtc` | ✅ Available — Device Tree Compiler (needed for DTB builds) |

#### 8.4.2 Ports Resolution Logic

When `emu blob fetch` is called, it should:

1. **Check if the blob is available via ports** — Look up the port name in a built-in mapping table
2. **Check if the port is installed** — Use `pkg info <port-name>` or check for the port's installed files
3. **If installed** — Create a **symlink** from the blob cache to the port's installed file. Do NOT copy the file.
4. **If not installed** — Suggest the user install the port: `pkg install <port-name>`
5. **If the port doesn't exist** — Fall back to direct download from upstream URL

```c
/* Ports resolution — check if blob is available via FreeBSD ports */
int
emu_blob_resolve_ports(const char *blob_id, const char *filename,
                       char *path, size_t pathlen)
{
    ...
    /* Look up port mapping for this blob */
    if (!emu_blob_port_lookup(blob_id, &port_path, &port_file))
        return (ENOENT);  /* No port mapping for this blob */

    /* Check if port is installed */
    if (stat(port_path, &sb) != 0)
        return (ENOENT);  /* Port not installed */

    /* Build path to port's installed file */
    snprintf(path, pathlen, "%s/%s", port_path, port_file);
    return (0);
}
```

#### 8.4.3 Port-to-Blob Mapping Table

The mapping is defined in `blobs.json` under a `ports` field:
...
```json
{
  "blobs": {
    "seabios": {
      "ports": {
        "pkg_name": "seabios",
        "port_path": "/usr/local/share/seabios/",
        "file_mapping": {
    ...
    "ovmf-x64": {
      "ports": {
        "pkg_name": "edk2-ovmf",
        "port_path": "/usr/local/share/edk2/",
        "file_mapping": {
...
When a blob is available via ports:

```
$ emu blob fetch ovmf-x64
INFO: OVMF (X64) is available via FreeBSD ports.
  Run: pkg install edk2-ovmf
  Then: emu blob fetch ovmf-x64  (to link from ports)

$ pkg install edk2-ovmf
[installs OVMF to /usr/local/share/edk2/]

$ emu blob fetch ovmf-x64
✓ Linked OVMF_CODE.fd from /usr/local/share/edk2/OVMF_CODE.fd
✓ Linked OVMF_VARS.fd from /usr/local/share/edk2/OVMF_VARS.fd
```

#### 8.4.5 Ports vs Direct Download Decision

| Condition | Action |
|-----------|--------|
| Port exists AND is installed | Create symlink from port installation |
| Port exists but is NOT installed | Suggest `pkg install <port>` |
| Port does NOT exist | Download from upstream URL |
| `--force` flag used | Download from upstream URL regardless of ports |
| Port version is outdated vs manifest | Warn user, suggest updating port |

#### 8.4.6 Ports Cache Integration

The ports symlinks are stored in the blob cache alongside downloaded blobs:

```
/var/emu/blobs/
  ├── ovmf-x64/
  │   ├── OVMF_CODE.fd → /usr/local/share/edk2/OVMF_CODE.fd  (symlink)
  │   └── OVMF_VARS.fd → /usr/local/share/edk2/OVMF_VARS.fd  (symlink)
```

This means the cache resolution order remains the same — the emulator doesn't need to know whether a blob came from ports or direct download.

### 8.5 Package Integration (Future)
