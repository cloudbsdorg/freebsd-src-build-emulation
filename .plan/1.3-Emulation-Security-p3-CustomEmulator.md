...
### 6.9 FreeBSD rctl Resource Limits

FreeBSD's `rctl` (resource limits) subsystem provides OS-level resource enforcement. The emulation framework integrates with rctl to enforce per-instance limits:

#### rctl Integration

The kernel module `emu_rctl.c` provides integration with FreeBSD's rctl subsystem:
...
/* Remove rctl limits from process */
int emu_rctl_remove(struct proc *p);

/* Get current rctl status */
int emu_rctl_status(void);
```

#### Supported rctl Limits

| Resource | Limit Type | rctl Rule | Enforcement |
|----------|------------|-----------|-------------|
| Memory per instance | Hard cap | `pid:<pid>:memoryuse:deny=<bytes>` | OS-enforced RSS limit |
| Virtual memory | Hard cap | `pid:<pid>:vmem:deny=<bytes>` | OS-enforced vmem limit |
| CPU time | Hard cap | `pid:<pid>:cputime:deny=<seconds>` | OS-enforced CPU limit |
| Process count | Hard cap | `pid:<pid>:nproc:deny=<count>` | Fork bomb prevention |
| Memory per user | Aggregate | `user:<uid>:memoryuse:deny=<bytes>` | Per-user aggregate |
| Process per user | Aggregate | `user:<uid>:nproc:deny=<count>` | Per-user aggregate |

#### Configuration Sysctls

| Sysctl | Default | Description |
|--------|---------|-------------|
| `kern.emulation.rctl.enabled` | 1 | Enable/disable rctl integration |
| `kern.emulation.rctl.default_memory_mb` | 16384 | Default memory limit per instance (MB) |
| `kern.emulation.rctl.default_cpu_seconds` | 86400 | Default CPU time limit (seconds) |
| `kern.emulation.rctl.default_max_procs` | 1024 | Default max processes per instance |

#### Instance PID Tracking

When an emulator process starts, it calls `emu_instance_attach_pid()` to register with the instance:
...
This allows the kernel module to apply rctl limits based on the instance's configured limits.
